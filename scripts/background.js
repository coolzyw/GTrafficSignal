...
chrome.runtime.onInstalled.addListener(function (callback) {
  if (callback.reason == "install") {
    ...
    var message = "Extension installed at:&nbsp&nbsp";
    message += timeStamp();
    console.log(message);
    addToLog(message);
  } 
    ...
    var message = "Extension updated at:&nbsp&nbsp";
    message += timeStamp();
    console.log(message);
    addToLog(message);
  }
...
chrome.storage.local.get(['log'], function (result) {
  ...
  window.setInterval(() => {
    ...
    if (signed_in == true && signed_in_display==false) {
      var message = "User signed in at:&nbsp&nbsp";
      message += timeStamp();
      console.log(message);
      addToLog(message);
    ...
    if (flag === false) {
      ...
      if (trueCondition !== prev_condition) {
        var message = "Internet offline at:&nbsp&nbsp";
        message += timeStamp();
        console.log(message);
        addToLog(message);
      }
      ...
      if (GFLAG === true) {
        trueCondition = "green";
        if (trueCondition !== prev_condition) {
          var message = "Google drive up at:&nbsp";
          message += timeStamp();
          console.log(message);
          addToLog(message);
        }
